...
import { iconMap } from './assembly-data.js';
import { aligned, localeMap } from './load-messages.js';

type Transformer = (data: { openMarkupTags: string[]; locale: string; parameters: string[] }) => string;

export const dump: DumpFunction = ({ html, targetDir, dataDir}) => {
	...
	const transformers: Record<string, Transformer> = {
		...
		'DBTN': ({ locale, parameters: [id] }) => `<span class="controller ${buttonLookup('joy', ensure(id), locale)[0]}"></span>`,
		'CBTN': ({ locale, parameters: [id] }) => `<span class="controller ${buttonLookup('joy', ensure(id), locale)[0]}"></span>`,
		'JCBT': ({ locale, parameters: [id] }) => `<span class="controller ${buttonLookup('joy', ensure(id), locale)[0]}"></span>`,
		'KCBT': ({ locale, parameters: [id] }) => {
			const [style, content = ''] = buttonLookup('key', ensure(id), locale);
		...
		'ICON': ({ locale, parameters: [id] }) => {
			...
			return `<span class="icon ${ensure(iconMap[ensure(id)]).replaceAll('_', '-')}"></span>`;
		},
	};
		...
		const transformedMessage = [...message.matchAll(/\[([A-Z0-9]+?)(?:=(.+?))?\]/g)].reduce(
			(result, [match, tag, parameters]) => {
				return result.replace(
					match,
					transformers[ensure(tag)]?.({ locale, openMarkupTags, parameters: parameters?.split(',') ?? [] }) ?? '',
				);
			},
			htmlEncode(message),
		);
