...
export class CSharpFile {
	...
	constructor(file: string | URL) {
		this.tree = parser.parse(readFileSync(file, 'utf-8'), undefined, { bufferSize: 1024 ** 2 });
	}
	...
	parseObjectField<T>(name: string) {
		return new Parser.Query(
			...
			.reduce<Record<string, T>>((result, _, index, source) => {
				if (index % 2 === 0) {
					const key = JSON.parse(ensure(source[index]?.node.text));
					result[key] = JSON.parse(ensure(source[index + 1]?.node.text));
				}
